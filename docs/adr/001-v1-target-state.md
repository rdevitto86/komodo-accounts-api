...
Customer-api is the sole owner of customer identity data for Komodo: profiles, account settings, passkey credential public keys, preferences, address books, payment-method references, consent history. It is a **data store, not an authenticator**: auth-api verifies credentials by reading them over the private plane (`GET /v1/users/credentials`) and issues all tokens (auth-api ADR 001).
...
| 6 | Customer-domain rename + business logic (settings, consent, export, unsubscribe, CDC) | Done (2026-06-27) |
...
Komodo leans into passwordless (passkeys + OTP) as the primary experience but **retains password login as a backup**. Consequences for customer-api:
...
- **Hashing stays out of customer-api** (PRD non-goal: not an authenticator). auth-api hashes plaintext (Argon2id) and writes the result via `PUT /v1/users/{id}/credentials`. customer-api never accepts a client-supplied precomputed hash and never hashes on a public route.
...
`POST /v1/communications/unsubscribe` (public, HMAC-authenticated). Mint endpoint: `POST /internal/v1/customers/{id}/communications/unsubscribe-token`.
...
`PUT /v1/me/addresses/{id}`, `PUT /v1/me/preferences`, `PUT /v1/customers/{id}/settings`, and `PUT /v1/me/profile` are all partial. Payment `PUT /v1/me/payments` stays full-replace upsert by ID (intentional — payment-method records are write-once metadata blobs from the PSP).
...
`DELETE /v1/me/profile` no longer hard-erases. It sets `Settings.status=pending_deletion`, `status_changed_at=now`, emits `customer.deletion_requested`, returns 202. `POST /v1/me/profile/restore` (authenticated, inside window) reverses. A scheduled worker hard-erases at `status_changed_at + 30d` (Query + chunked `TransactWriteItems` ≤100 + S3 exports + S3 avatars + cache invalidation, emits `customer.deleted`).
...
`POST /v1/me/profile/avatar` returns a 15-min pre-signed S3 PUT URL into `s3://komodo-customer-avatars-<env>/<customer_id>/<ksuid>.{jpg|png|webp}`. Client uploads direct to S3 (≤2 MB, `Content-Type` constrained), then `PUT /v1/me/profile {avatar_url: ...}`. Reads are also pre-signed GETs (no public objects).
...
- `User.middle_initial` — no business logic depends on it; ~30% of US users have none; if shipping labels ever need it, model on `Address` (`recipient_middle_initial`), not on identity.
...
- **Why:** Settings has multiple cross-service writers (loyalty, marketing, support, customer-servicing) — last-write-wins loses tag and verified-flag state. `is_default` had a race window that allowed two-defaults or zero-defaults.
...
### D8 — This service owns the customer DynamoDB table (CDK)

`deploy/cdk/main.ts` uses `new dynamodb.Table(...)` — **never** `Table.fromTableName(...)`. Canonical spec: PK/SK String, GSI1 with `INCLUDE` + `customer_id`, `PAY_PER_REQUEST`, `NEW_AND_OLD_IMAGES` streams, PITR, AWS-managed KMS, deletion-protection on stg/prod, `RemovalPolicy.RETAIN` on stg/prod. Stream ARN exported as `${env}-CustomersTableStreamArn` for events-api consumption.

- **Why:** `fromTableName` makes the schema opaque to this stack — `cdk diff` cannot catch drift, stream ARN can't be cleanly exported, ownership of streams/PITR/encryption is ambiguous. The customer domain owns its data store end-to-end.
...
Realistic per-customer counts in V1: addresses ≤5, payment methods ≤3, passkeys ≤5, consent log ≤dozens. The 100-item cap sits well above the long tail. When V2 lands cursor pagination, ship `{items, next_cursor}` wrapper across all list endpoints in one contract change.
...
| Replace in-process TTL cache with Redis/ElastiCache in V1 | Rejected | In-process at 100k entries + 60 s TTL meets the p99 budget at 10M users. Revisit at 50M or multi-region. |
...
| Reject `UpdateUser` requests that carry immutable fields with 400 | Rejected (silent-drop chosen) | Lets clients round-trip the full object without surgically stripping `user_id` / `email` / `created_at` / etc. Drop is documented in the OpenAPI `UpdateUserRequest` description. |
...
- D0 keeps customer-api a pure data store: auth-api owns hashing and verification, customer-api owns storage and the read/write contract. Preserves the PRD's "not an authenticator" non-goal while supporting password login.
...
- D8 makes this repo the single source of truth for the customers table schema. Any future schema change starts in `data-model.md` and the CDK in the same PR.
...
- After Phase 9, customer-api enters GA-candidate state. Forward-looking work (avatar transforms, cursor pagination, shareable wishlists, multi-region cache) is V2 and out of scope here.
